...
import { chartOptionsDefaults } from './model/default-options';
import { SymbolDatas, CIntervals } from './model/constants';
...
class App {
  ...
  reRender = (shapes, flag) => {
    if (!flag) {
      this._chartWidget._pan._global_autosave_flag = false;
    }
    this._chartWidget._pan.initPen();
    this._chartWidget.renderShapes(shapes);
  ...
  drawTimeLines = (xTime) => {
    if (!this._chartWidget._loaded || !this._chartWidget._pan) return false;
    if (this._chartWidget._pan && typeof this._chartWidget._pan.drawTimeLines == 'function') {
  ...
  hideTimeLines = () => {
    ...
    if (this._chartWidget._pan && typeof this._chartWidget._pan.hideTimeLines == 'function') {
