...
import { SidebarWidget } from './left-sidebar';
import { TopToolbarWidget } from './top-toolbar';
import { PanWidget } from './pan-widget';
import { ChatWidget } from './chat-widget';
import { colorPalette, leftTools, CIntervals } from '../model/constants';
import {
  CreateUUID
...
export class ChartWidget {
  ...
  constructor(container, options) {
    ...
    if (this._options.writerId && parseInt(this._options.writerId) > 0) {
      // this.loaded(false);
      $.get({
        url: `${options.hostUrl}/api/layouts/default?userId=${options.writerId}&theme=${options.defaultTheme}`,
        dataType: "json",
        ...
        success: res => {
          if (res.error) {
            _.merge(that._options, { _id: null });
            that._initialize();
  ...
  saveChart = (type, cb) => {
    ...
    if (type === 'signal') {
      this._pan.takeSnapshotImage((imageUrl) => {
        postSave({ data: JSON.stringify(reqOptions), imageUrl: imageUrl });
      }, true);
    } else {
      postSave({ data: JSON.stringify(reqOptions) });
    }
  ...
  _drawToolbars = () => {
      ...
      $(toolbar).find(`.color-picker[key="${wrapper}"]`).spectrum({
        ...
        change: color => {
          ...
          if (wrapper == 'backgroundColor') {
            ...
            option = {
              mainLine: { lineColor: rgbColor }
            }
            if (that._pan._pen.objectType.indexOf('fib') >= 0) {
              _.merge(option, { levelLine: { color: rgbColor } });
              for (let i = 0; i < 10; i++) {
      ...
      .popup({
        ...
        onVisible: function () {
          $('.my-popup.line-style .ui.button').click(function (e) {
            ...
            var option = {
              mainLine: { lineDashType: ls[parseInt(val)] }
            };
            that._pan.updateShape(option, true);
            var lineStyle = [
              '<path d="M1.5 2a.5.5 0 0 0 0 1h19a.5.5 0 0 0 0-1h-19z"></path>',
      ...
      .popup({
        ...
        onVisible: function () {
          $('.my-popup.start-arrow .ui.button').click(function (e) {
            var val = $(e.target).closest('.ui.button').attr('key');
            var option = {
              start: { arrow: val == 1, extend: val == 2 }
            };
            that._pan.updateShape(option, true);
            var arrows = [
    ...
    $(toolbar).find('.tv-floating-toolbar__widget[key="lock"]').click((e) => {
      ...
      that._pan.updateShape({ isLocked: !flag }, true);
      if (flag)
    ...
    $(toolbar).find('.tv-floating-toolbar__widget[key="eye"]').click((e) => {
      ...
      that._pan.updateShape({ visible: !flag }, true);
      if (!flag)
        $(e.target).closest('.tv-floating-toolbar__widget').find('.i-active').removeClass('i-active');
      else
        $(e.target).closest('.tv-floating-toolbar__widget').find('.apply-common-tooltip').addClass('i-active');
