...
export class ChartPan {
  ...
  _addCanvas = (flag = true) => {
    if (this._component && $(this._component.container).find('canvas').length === 2) {      
      ...
      if (flag) {
        ...
        const chartElem = document.getElementById("primary_chart");
        chartElem.addEventListener('mousemove', this.handleMouseMove);
        chartElem.addEventListener('mouseup', this.handleMouseUp);
        chartElem.addEventListener('mousedown', this.handleMouseDown);
    ...
    if (flag) {
      ...
      setTimeout(() => {
        ...
        $(hideTrialArea).addClass('hide-trial-area').css('width', 55).css('height', 10).css('position', 'absolute').css('left', 1).css('bottom', 2)
  ...
  draw = (pen) => {
    ...
    let temps = chart.options.axisY2.stripLines.filter(o => (['price', 'final-price', 'price-width', 'trade'].indexOf(o.objectType) >= 0));
    ...
    if (this._shapes && this._shapes.length > 0) {
      const filterOptions = this._shapes.filter(option => {
        if (option.key != 'h-ray' && !option.indicator && ['analysis', 'trade'].indexOf(option.key) < 0 && option.timeRange.length > 0) {
          let inFlag = false;
      ...
      sortOptions.forEach(option => {
        ...
        if (option.key === 'h-ray') drawHRay(this._ctx, option, points, lastViewX, chart, this._precision);
  ...
  handleMouseDown = e => {
    ...
    if (this._parent._pen && this._parent._pen.enabled) {
      ...
      if (detection.selectEnabled) {
        ...
        this._parent.shapeSelected({option: detection.option, oIndex: detection.oIndex});
  ...
  _detectPosition = point => {
    ...
    let options = this._shapes.filter(option => {
      ...
      if (!option.indicator && ['trade', 'analysis'].indexOf(option.key) < 0 && option.timeRange.length > 0) {
        let inFlag = false;
        inFlag = option.timeRange[0] >= minViewport && option.timeRange[0] <= maxViewport;
        inFlag = inFlag || option.timeRange[1] >= minViewport && option.timeRange[1] <= maxViewport;
        inFlag = inFlag || (option.timeRange[0] >= minViewport && option.timeRange[1] <= maxViewport);
  ...
  _makeIndicatorTool = (optionId) => {
    ...
    $(wrapper).find(`.buttons[key="${option.id}"] .button`).on('pointerenter', (e) => {
      ...
      const option = that._shapes.find(o => (o.id === oId));
