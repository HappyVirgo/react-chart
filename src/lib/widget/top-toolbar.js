...
export class TopToolbarWidget {
	...
	_renameLayout = () => {
		...
		$(this._element).find('.modal-body').dialog({
			...
			open: event => {
				...
				$(modalEl).find('#chart_name').val(that._chart._layout.name);
				that.shared(false);
				$(modalEl).find('.button.ok').click(function () {
					let chartName = $(modalEl).find('#chart_name').val();
					if (chartName.length > 0 && chartName != that._chart._layout.name) {
						if (!confirmed) {
							confirmed = true;
							$.get({
								url: `${options.hostUrl}/api/layouts/checkname?userId=${options.writerId}&name=${chartName}&id=${that._chart._layout_id}`,
								dataType: "json",
								...
								success: res => {
									...
									$(modalEl).find('.header.for-confirm h2').html(`Do you want to rename "${options.name}" layout to "${chartName}"?`)
	...
	setSymbol = (symbol) => {
		...
		const layoutShapes = this._chart._layout.shapes && this._chart._layout.shapes[symbol.name];
	...
	_renderLayoutLists = (modalEl, refresh = false) => {
		...
		$.get({
			url: `${option.hostUrl}/api/layouts?userId=${option.writerId}`,
			dataType: "json",
			beforeSend: function (xhr) {
