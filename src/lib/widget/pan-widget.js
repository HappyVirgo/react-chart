...
export class PanWidget {
  ...
  _rangeChanged = e => {
    ...
    if (this._minViewport < this._mainPoints[50].x.getTime() && !this._gettingFlag) {
      console.log(new Date(this._minViewport), this._mainPoints[50].x);
      this._gettingFlag = true;
  ...
  _initializeSetting = () => {
    ...
    const panIndex = ['primary_chart', 'extra_0', 'extra_1'].indexOf(this._pen.option.containerId);
    const chart = this._pans[panIndex < 0 ? 0 : panIndex]._component;
  ...
  setShapes = (newShapes) => {
    ...
    const extraNums = this._shapes.filter(o => (o.extra && !o.comment)).length;

    containers.forEach(containerId => {
      let curPan = that._pans.find((pan) => (pan._containerId === containerId));
      if (curPan) {
        curPan._shapes = shapes[containerId];
        curPan.draw();
      } else {
  ...
  setRange = (range) => {
    if (range) {
      ...
      this._pans.forEach(pan => {
        pan._component.axisX[0].set('viewportMinimum', this._viewport.x[0], false);
        pan._component.axisX[0].set('viewportMaximum', this._viewport.x[1], false);
        pan._component.set("dataPointWidth", stickWidth);
        pan.draw();
      });
    }
