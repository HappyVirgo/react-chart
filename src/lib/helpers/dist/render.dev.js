...
exports.detectLine = detectLine;
exports.getMiddle = getMiddle;
exports.getTextStyle = getTextStyle;
exports.drawSelecter = drawSelecter;
exports.drawHRay = drawHRay;
exports.drawArrow = drawArrow;
exports.drawRect = drawRect;
exports.drawFib = drawFib;
exports.drawCallout = drawCallout;
exports.drawXABCD = drawXABCD;
exports.drawABCD = drawABCD;
exports.drawTriPattern = drawTriPattern;
exports.drawThreePattern = drawThreePattern;
exports.drawHeadShoulder = drawHeadShoulder;
exports.drawElliottWave = drawElliottWave;
exports.drawMAIndicator = drawMAIndicator;
exports.drawIchimokuIndicator = drawIchimokuIndicator;
exports.drawAnalysis = drawAnalysis;
exports.drawTradeLines = drawTradeLines;
exports.drawParallel = drawParallel;
...
function detectLine(point, points) {
  var res = {
    flag: false,
    pointIndex: -1
  };
...
function drawTextWithBackground(ctx, text, points, option, flag) {
  ...
  if (flag > 0) point.y = point.y - rectHeight / 2 - 3;else if (flag < 0) point.y = point.y + rectHeight / 2 + 3;
  ctx.beginPath();
  ctx.globalAlpha = 1;
  ...
  ctx.setLineDash([]);
  ctx.fillStyle = option.color;
  ctx.font = "".concat(option.fontStyle, " ").concat(option.fontWeight, " ").concat(option.fontSize, "px ").concat(option.fontFamily);
  ctx.fillText(text, point.x - textOffset.x / 2, point.y + rectHeight / 3);
}

function drawMultiRange(ctx, points, option) {
  var hasBorder = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
function getMiddle(data, index, offset) {
  ...
  var result = (Math.max.apply(Math, highest) + Math.min.apply(Math, lowest)) / 2;
  if (!flag) result = {
    high: Math.max.apply(Math, highest),
    low: Math.min.apply(Math, lowest)
  };
  return result;
}
/**
...
function drawSplineRange(ctx, dataPoints, color, chart) {
  ...
  for (i = 1; i < dataPoints.length - 2; i++) {
    ...
    var yc = (dataPoints[i].y[0] + dataPoints[i + 1].y[0]) / 2;
    ctx.quadraticCurveTo(chart.axisX[0].convertValueToPixel(dataPoints[i].x), chart.axisY2[0].convertValueToPixel(dataPoints[i].y[0]), xc, chart.axisY2[0].convertValueToPixel(yc));
  } // curve through the last two points


  ctx.quadraticCurveTo(chart.axisX[0].convertValueToPixel(dataPoints[i].x), chart.axisY2[0].convertValueToPixel(dataPoints[i].y[0]), chart.axisX[0].convertValueToPixel(dataPoints[i + 1].x), chart.axisY2[0].convertValueToPixel(dataPoints[i + 1].y[0]));
  dataPoints.reverse();
  ctx.lineTo(chart.axisX[0].convertValueToPixel(dataPoints[0].x), chart.axisY2[0].convertValueToPixel(dataPoints[0].y[1]));

  for (i = 1; i < dataPoints.length - 2; i++) {
    var xb = (chart.axisX[0].convertValueToPixel(dataPoints[i].x) + chart.axisX[0].convertValueToPixel(dataPoints[i + 1].x)) / 2;
    var yb = (dataPoints[i].y[1] + dataPoints[i + 1].y[1]) / 2;
    ctx.quadraticCurveTo(chart.axisX[0].convertValueToPixel(dataPoints[i].x), chart.axisY2[0].convertValueToPixel(dataPoints[i].y[1]), xb, chart.axisY2[0].convertValueToPixel(yb));
  } // curve through the last two points


  ctx.quadraticCurveTo(chart.axisX[0].convertValueToPixel(dataPoints[i].x), chart.axisY2[0].convertValueToPixel(dataPoints[i].y[1]), chart.axisX[0].convertValueToPixel(dataPoints[i + 1].x), chart.axisY2[0].convertValueToPixel(dataPoints[i + 1].y[1]));
  ctx.closePath();
  ctx.globalAlpha = 0.2;
  ctx.fillStyle = color;
  ctx.fill();
...
function drawSpline(ctx, dataPoints, option, chart) {
  ...
  ctx.setLineDash([]);
  var i = 0;
  var convertedPoints = dataPoints.map(function (point) {
    return {
  ...
  for (i = 1; i < dataPoints.length - 2; i++) {
    ...
    ctx.quadraticCurveTo(convertedPoints[i].x, convertedPoints[i].y, xc, yc);
  } // curve through the last two points


  ctx.quadraticCurveTo(convertedPoints[i].x, convertedPoints[i].y, convertedPoints[i + 1].x, convertedPoints[i + 1].y);
  ctx.stroke();
  return convertedPoints;
}

function getTextStyle(text, option) {
...
function drawSelecter(ctx, points) {
...
function drawHRay(ctx, option, points, lastX, chart, precision) {
  if (points.length > option.pointsNum - 1) {
    var dashType = [];
    if (option.mainLine.lineDashType === "dot") dashType = [3, 3];else if (option.mainLine.lineDashType === "dash") dashType = [10, 5];
    var endP = {
      x: lastX,
      y: points[0].y
    };
    ...
    if (option.label.visible && option.label.text.length > 0) {
      ...
      drawText(ctx, option.label.text, labelX, labelY, {
        ...
        font: "".concat(option.label.fontStyle, " ").concat(option.label.fontWeight, " ").concat(option.label.fontSize, "px ").concat(option.label.fontFamily)
      });
    }
    ...
    if (option.label.indexVisible) {
      ...
      var priceIndex = chart.options.axisY2.stripLines.findIndex(function (o) {
        return o.optionId === option.id;
      });
...
function drawArrow(ctx, option, points, chart, selected, lastViewX, minViewX) {
  if (points.length > option.pointsNum - 1) {
    var dashType = [];
    if (option.mainLine.lineDashType === "dot") dashType = [3, 3];else if (option.mainLine.lineDashType === "dash") dashType = [10, 5];
    var lineOption = {
    ...
    var p0 = points[0];
    var p1 = points[1];
    var y0 = (p0.x * p1.y - p1.x * p0.y) / (p0.x - p1.x);
    var y1 = ((lastViewX - p0.x) * (p1.y - p0.y) + (p1.x - p0.x) * p0.y) / (p1.x - p0.x);
    var minP = p0.x < p1.x ? p0 : p1;
    var maxP = p0.x < p1.x ? p1 : p0;
    var sP = option.start.extend ? {
      x: minViewX,
      y: y0
    } : minP;
    var eP = option.end.extend ? {
      x: lastViewX,
      y: y1
    } : maxP;
    ...
    if (option.label.extra && selected) {
      ...
      if (option.label.extra.bars) {
        var temp_data = chart.options.data[0].dataPoints.filter(function (point) {
          if (point.x >= option.chartPoints[0].x && point.x <= option.chartPoints[1].x) return true;else if (point.x <= option.chartPoints[1].x && point.x >= option.chartPoints[1].x) return true;else return false;
        });
      ...
      if (option.label.extra.date) {
        ...
        if (option.label.extra.bars) labelText += '(' + Math.round(minutes) + 'min)';else labelText += Math.round(minutes) + 'min      ';
      }

      if (labelText.length > 0) subLabels.push(labelText);
      ...
      if (startP.y < endP.y) labelY -= textOffset.y * labels.length;else labelY = labelY + textOffset.y + 5; // drawing rectangle of text pane
      ...
      labels.forEach(function (ll, index) {
...
function drawRect(ctx, option, points) {
...
function drawFib(ctx, option, tpoints, chart, precision, lastViewX) {
  // if (points.length < 2) return;
  var points = _toConsumableArray(tpoints);

  if (option.revert && option.key === "fibonacci") points.reverse();
  var prevPoint = {}; // ctx.globalAlpha = 0.3;

  if (points.length > option.pointsNum - 1) {
    var offsetY = points[0].y - points[1].y;
    var dashType = [];
    if (option.levelLine.lineDashType === "dot") dashType = [3, 3];else if (option.levelLine.lineDashType === "dash") dashType = [10, 5];
    var seedNum = Math.pow(10, precision);
    var sortedLevels = option.levels.filter(function (level) {
    ...
    for (var i = 0; i < sortedLevels.length; i++) {
      ...
      if (option.key === 'fibonacci') {
        ...
        if (option.revert) offsetY = -1 * offsetY;
      ...
      var hover = option.sub_hovers && option.sub_hovers.indexOf(level.value.toString()) >= 0;
      ...
      drawText(ctx, labelText, labelX, labelY, {
        color: level.color,
        font: "normal normal ".concat(option.label.fontSize, "px Arial")
      });
  ...
  var mainDashType = [];
  if (option.mainLine.lineDashType === "dot") mainDashType = [3, 3];else if (option.mainLine.lineDashType === "dash") mainDashType = [10, 5];
...
function drawCallout(ctx, option, points) {
  var labels = option.label.text.split("\n");
  var labelLength = labels.map(function (item) {
    return item.length;
  });
  var maxLength = Math.max.apply(Math, _toConsumableArray(labelLength));
  var maxLabel = labels[labelLength.indexOf(maxLength)];
  var textOffset = getTextStyle(maxLabel, option.label);
  ...
  if (insideFlag) {
    ...
    if (alpaOffsetX < 0) {
      if (alpaOffsetY < 0) alpa = Math.PI - alpa;else alpa = Math.PI + alpa;
    } else {
      if (alpaOffsetY > 0) alpa = 2 * Math.PI - alpa;
    }
  ...
  ctx.setLineDash([]); // ctx.globalAlpha = option.background.opacity;
...
function drawXABCD(ctx, option, points) {
  if (points.length < 2) return;
  ctx.setLineDash([]);
...
function drawABCD(ctx, option, points) {
  if (points.length < 2) return;
  ctx.setLineDash([]);
  ctx.strokeStyle = option.mainLine.lineColor;
  ctx.lineCap = 'round';
...
function drawTriPattern(ctx, option, points) {
  if (points.length < 2) return;
  ctx.setLineDash([]);
  ctx.strokeStyle = option.mainLine.lineColor;
  ctx.lineCap = 'round';
  ...
  var xPoints = points.map(function (point) {
    return point.x;
  });
  var labelOption = Object.assign(option.label, {
    lineColor: option.mainLine.lineColor
  ...
  if (points.length > 3) {
    ...
    var minXVal = Math.min.apply(Math, _toConsumableArray(xPoints));
    var maxXVal = Math.max.apply(Math, _toConsumableArray(xPoints));
    ...
    if (m0 !== m1) {
      var trianglePoints = [{
        x: xVal,
        y: [yVal, yVal]
      }].concat(_toConsumableArray(lastPoint));
      var newPoints = [{
...
function drawThreePattern(ctx, option, points) {
  if (points.length < 2) return;
  ctx.setLineDash([]);
  ctx.strokeStyle = option.mainLine.lineColor;
  ctx.lineCap = 'round';
...
function drawHeadShoulder(ctx, option, points, lastX, firstX) {
  ...
  for (var i = 1; i < points.length; i++) {
    drawLine(ctx, points[i - 1], points[i], lineOption);
    if (i % 2) drawTextWithBackground(ctx, titles[Math.round(i / 2) - 1], [points[i]], labelOption, points[i - 1].y - points[i].y);
  }

  var xPoints = points.map(function (point) {
    return point.x;
  });
  var minX = Math.min.apply(Math, _toConsumableArray(xPoints));
  var maxX = Math.max.apply(Math, _toConsumableArray(xPoints));
  ...
  if (points.length > 4) {
    ...
    if (points.length > 6) {
      ...
      var y2 = points[6].y + m2 * (x2 - points[6].x);
...
function drawElliottWave(ctx, option, points) {
  if (points.length < 2) return;
  ctx.setLineDash([]);
  ctx.strokeStyle = option.mainLine.lineColor;
  ctx.lineCap = 'round';
...
;
var source = ['open', 'high', 'low', 'close'];

function drawMAIndicator(ctx, dataPoints, option, chart) {
  var key = option.key;
  var tempPoints = [];
  ...
  for (var i = option.len - 1; i < dataPoints.length; i++) {
    ...
    for (var j = i + 1 - option.len; j <= i; j++) {
      if (key === 'ma') subsum += dataPoints[j].y[source.indexOf(option.source)];else if (key === 'wma') {
    ...
    if (key === 'ma') result = subsum / option.len;else if (key === 'wma') result = subsum / totalWeight;else if (key === 'ema') {
      var pema = subsum / option.len;
      if (i < option.len) pema = dataPoints[i - 1].y[source.indexOf(option.source)];
...
function drawIchimokuIndicator(ctx, tempPoints, setting, chart) {
  var tenkan = [];
  var kijun = [];
  var senkouA = [];
  var senkouB = [];
  ...
  for (var i = 0; i < tempPoints.length; i++) {
    ...
    if (offset < setting.conversionPeriod) offset++;else offset = 0;
...
function drawAnalysis(ctx, option, points, lastViewX, minViewX, chart, precision) {
  var newDraw = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
  var dataPoints = arguments.length > 8 ? arguments[8] : undefined;
  var margin = arguments.length > 9 ? arguments[9] : undefined;
  var tooltipFlag = arguments.length > 10 ? arguments[10] : undefined;
    ...
    if (parseInt(option.selected) >= 0) {
      if (option.selected == 0) {
        ...
        if (validFlag = valY > option.signals[8].value.y) {
          ...
          msg = "<font color=\"".concat(option.signals[0].color, "\">Support</font> value  cannot be <b>above</b> the<br/><font color=\"").concat(option.signals[8].color, "\">Pivot</font> value (Move cursor down)");
          direction = 'down';
        } else if (validFlag = option.signals[1].value.y > 0 && valY < option.signals[1].value.y) {
          valY = option.signals[1].value.y;
          msg = "<font color=\"".concat(option.signals[0].color, "\">First Support</font> value  cannot be <b>below</b> the<br/><font color=\"").concat(option.signals[1].color, "\">Second Support</font> value (Move cursor up)");
          direction = 'up';
        } else if (validFlag = option.signals[3].value.y > 0 && valY > option.signals[3].value.y) {
          valY = option.signals[3].value.y; // msg = `<font color="${option.signals[0].color}">First Support</font> value  cannot be <b>below</b> the <font color="${option.signals[1].color}">Second Support</font> value(Move cursor up)`;
        }
        ...
        if (validFlag = valY > option.signals[0].value.y) {
          ...
          msg = "<font color=\"".concat(option.signals[1].color, "\">Second Support</font> value  cannot be <b>above</b> the<br/><font color=\"").concat(option.signals[0].color, "\">First Support</font> value (Move cursor down)");
          direction = 'down';
        }
        ...
        if (validFlag = valY > option.signals[8].value.y) {
          valY = option.signals[8].value.y;
          msg = "<font color=\"".concat(option.signals[2].color, "\">").concat(option.signals[2].title, "</font> value  cannot be <b>above</b> the<br/><font color=\"").concat(option.signals[8].color, "\">Pivot</font> value (Move cursor down)");
          direction = 'down';
        } else if (validFlag = valY < option.signals[0].value.y) {
          valY = option.signals[0].value.y;
          msg = "<font color=\"".concat(option.signals[2].color, "\">").concat(option.signals[2].title, "</font> value  cannot be <b>below</b> the<br/><font color=\"").concat(option.signals[0].color, "\">First Support</font> value (Move cursor up)");
          direction = 'up';
        }
      } else if (option.selected == 3) {
        if (option.signals[0].value.y == 0) {
          var between = chart.axisY2[0].convertPixelToValue(0) - chart.axisY2[0].convertPixelToValue(30);
          option.signals[0].value.y = parseFloat(Number(valY - between).toFixed(precision));
        }

        if (validFlag = valY < option.signals[0].value.y) valY = option.signals[0].value.y;else if (validFlag = valY > currentVal.y) valY = currentVal.y;
      } else if (option.selected == 7) {
        if (validFlag = option.signals[8].value.y <= 0) option.signals[8].value.y = currentVal.y;

        if (validFlag = valY < option.signals[8].value.y) {
          valY = option.signals[8].value.y;
          msg = "<font color=\"".concat(option.signals[7].color, "\">Resistance</font> value  cannot be <b>below</b> the<br/><font color=\"").concat(option.signals[8].color, "\">Pivot</font> value (Move cursor up)");
          direction = 'up';
        } else if (validFlag = option.signals[6].value.y > 0 && valY > option.signals[6].value.y) {
          valY = option.signals[6].value.y;
          msg = "<font color=\"".concat(option.signals[7].color, "\">First Resistance</font> value  cannot be <b>above</b> the<br/><font color=\"").concat(option.signals[6].color, "\">Second Resistance</font> value (Move cursor down)");
          direction = 'down';
        ...
        if (validFlag = valY < option.signals[7].value.y) {
          ...
          msg = "<font color=\"".concat(option.signals[6].color, "\">Second Resistance</font> value  cannot be <b>below</b> the<br/><font color=\"").concat(option.signals[7].color, "\">First Resistance</font> value (Move cursor up)");
          direction = 'up';
        }
        ...
        if (validFlag = valY < option.signals[8].value.y) {
          valY = option.signals[8].value.y;
          msg = "<font color=\"".concat(option.signals[5].color, "\">").concat(option.signals[5].title, "</font> value  cannot be <b>below</b> the<br/><font color=\"").concat(option.signals[8].color, "\">Pivot</font> value (Move cursor up)");
          direction = 'up';
        } else if (validFlag = valY > option.signals[7].value.y) {
          valY = option.signals[7].value.y;
          msg = "<font color=\"".concat(option.signals[5].color, "\">").concat(option.signals[5].title, "</font> value  cannot be <b>above</b> the<br/><font color=\"").concat(option.signals[7].color, "\">First Resistance</font> value (Move cursor down)");
          direction = 'down';
        }
      } else if (option.selected == 4) {
        if (option.signals[7].value.y == 0) {
          var _between = chart.axisY2[0].convertPixelToValue(0) - chart.axisY2[0].convertPixelToValue(30);

          option.signals[7].value.y = parseFloat(Number(valY + _between).toFixed(precision));
        }

        if (validFlag = valY > option.signals[7].value.y) valY = option.signals[7].value.y;else if (validFlag = valY < currentVal.y) valY = currentVal.y;
        if (option.signals[7].value.y == 0) option.signals[7].value.y = currentVal.y;
  ...
  for (var i = 0; i < 9; i++) {
    ...
    if (i != parseInt(option.selected) && option.signals[i].value.y != 0) {
      ...
      var valX;
      if (points.length <= 0) valX = option.signals[option.selected].value.x;
      if (point) valX = chart.axisX[0].convertPixelToValue(point.x);
    ...
    if (option.signals[i].visible) {
      ...
      if (option.signals[i].value.y != 0 && realY < chart.height - 45 && realY > 0) {
        ...
        var obj = $(".analysis_".concat(option.id, "_") + i);

        if (obj.length > 0) {
          obj.css("top", parseInt(realY) + 10).css("display", option.signals[i].value.y != 0 && option.signals[i].visible ? "inline-flex" : "none");
          $(".analysis_".concat(option.id, "_") + i).rating('set rating', option.signals[i].rate);
        } else {
          var rateObj = document.createElement("div");
          $(rateObj).addClass("analysis_".concat(option.id, "_") + i).addClass("ui rating star analyze");
      ...
      $(".analysis_".concat(option.id, "_") + i).css("display", "none");
      continue;
    }
  }

  if (stripLine) {
    var ttStripLines = chart.options.axisY2.stripLines;
    var tIndex = ttStripLines.findIndex(function (o) {
      return o.optionId === option.id && o.pIndex == parseInt(option.selected);
    });
  ...
  if (validFlag && msg.length > 0 && !tooltipFlag) {
    var tooltipElm = $("#".concat(option.containerId, " .analysis-tooltip"));
    ...
    if (tooltipElm.length > 0) {
      ...
      $(tooltipElm).addClass("analysis-tooltip ".concat(direction, " active"));
      ...
      $("#".concat(option.containerId, " .canvasjs-chart-container")).append(tooltipElm);
    ...
    if (selectedIndex == 2 || selectedIndex == 5) {
      ...
      if (selectedIndex < 7) minWidth = 330;else minWidth = 300;
    ...
    if (xPos > xLimit) {
      ...
      var _offset = point.x - xLimit;

      if (_offset > minWidth - 30) {
        _offset = minWidth - 30;
      }

      $(tooltipElm).find('.b-arr').css('left', _offset - 5);
    } else {
    ...
    $("#".concat(option.containerId, " .analysis-tooltip")).removeClass('active');
  }
...
function drawTradeLines(ctx, option, points, lastViewX, minViewX, chart, precision, dataPoints, lastX, finalPrice, margin) {
  $('.analyze').css('display', 'none');
  var point = points[0];
  var valY = 0; // setting current value of d 
  ...
  if (points.length > 0) {
    ...
    if (option.selected >= 0) {
      if (option.selFlag) {
        if (option.selected == 1 && valY < option.signals[0].value.y) valY = option.signals[0].value.y;else if (option.selected == 2 && valY > option.signals[0].value.y) valY = option.signals[0].value.y;else if (option.selected == 3 || option.selected == 5 || option.selected == 6) {
          if (valY < option.signals[0].value.y + option.signals[2].value.y) valY = option.signals[0].value.y + option.signals[2].value.y;else if (valY > option.signals[0].value.y) valY = option.signals[0].value.y;
        } else if (option.selected == 4) {
          if (valY < option.signals[0].value.y) valY = option.signals[0].value.y;else if (valY > option.signals[0].value.y + option.signals[1].value.y) valY = option.signals[0].value.y + option.signals[1].value.y;
        }
      } else {
        if (option.selected == 1 && valY > option.signals[0].value.y) valY = option.signals[0].value.y;else if (option.selected == 2 && valY < option.signals[0].value.y) valY = option.signals[0].value.y;else if (option.selected == 3 || option.selected == 5 || option.selected == 6) {
          if (valY > option.signals[0].value.y + option.signals[2].value.y) valY = option.signals[0].value.y + option.signals[2].value.y;else if (valY < option.signals[0].value.y) valY = option.signals[0].value.y;
  ...
  var ttStripLines = chart.options.axisY2.stripLines;
  var tempStripLines = ttStripLines.filter(function (o) {
    return o.optionId !== option.id;
  });
  ...
  chart.render(); // drawing signals

  if (option.signal.visible) {
    var tempPoints = _toConsumableArray(dataPoints);

    tempPoints.reverse();
    var createdAt = tempPoints.find(function (pp) {
      return pp.x <= option.signal.createdAt;
    });
    ...
    if (createdAt) {
      ...
      var used = option.signals.filter(function (signal) {
        return signal.used;
      });

      if (used.length > 0) {
        var entryReached = used[0];
        var otherReached = used.length > 1 ? used[1] : null;

        var _textOffset = getTextStyle(entryReached.title + " reached!", {
          fontSize: 12
        ...
        drawText(ctx, entryReached.title + " reached!", labelX - _textOffset.x / 2, labelY - 7, {
        ...
        if (otherReached) {
          ...
          ctx.fill();
          var otherTOffset = getTextStyle(otherReached.title + " reached!", {
            fontSize: 12
          });
...
function drawParallel(ctx, option, points, lastViewX) {
  if (points.length < 2) return;
  var mainDashType = [];
  if (option.mainLine.lineDashType === "dot") mainDashType = [5, 5];else if (option.mainLine.lineDashType === "dash") mainDashType = [10, 15];
  var lineOption = {
    thickness: option.hoverover ? 4 : parseInt(option.mainLine.lineThickness),
    dashType: mainDashType,
    color: option.hoverover ? option.hoverColor : option.mainLine.lineColor
  ...
  if (points.length > 2) {
    ...
    var expectY = kVal * (points[2].x - points[0].x) + points[0].y;
    offsetY = points[2].y - expectY;
  }

  var startP = points[0].x < points[1].x ? points[0] : points[1];
  var endP = points[0].x < points[1].x ? points[1] : points[0];
  ...
  if (points.length > 2) {
    var expectedPoints = [startP, endP, {
      x: endP.x,
      y: endP.y + offsetY
    }, {
      x: startP.x,
